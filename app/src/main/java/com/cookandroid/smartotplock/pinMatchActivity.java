...
public class pinMatchActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.pin_match);
                    ...
                    if(j>=0) {
                        ...
                        if (j == 4) {    // 사용자가 4자리의 수를 입력했을 때
                            ...
                            if (pincheck.equals(pin2)) {    // 처음 설정한 pin이 저장된 pincheck와 입력한 4자리 수가 같을 경우
                                Intent intent = new Intent(getApplicationContext(), pinChangeActivity.class);  // 패턴 설정 화면으로 이동
                                startActivity(intent);
                                finish();
                            } else {    // pin을 잘못 입력한 경우
                                // 틀릴 때마다 진동 울리기
